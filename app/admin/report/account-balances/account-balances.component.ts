...
import { AccountBalanceReport, AccountBalanceReportItem } from 'src/app/models/account-balance-report';
...
export class AccountBalancesComponent implements OnInit {
  assetsReports: AccountBalanceReport[] = [];
  liabilitiesReports: AccountBalanceReport[] = [];
  equityReports: AccountBalanceReport[] = [];
  incomeReports: AccountBalanceReport[] = [];
  expenseReports: AccountBalanceReport[] = [];
  reports: AccountBalanceReport[];
  ...
  getAccountBalanceReport() {
    this.loading = true;
    return this.reportService.getAccountBalanceReport(this.startAt, this.endAt).subscribe(
      result => {
        if (result != null) {
          this.reports = result;
          this.loading = false;
  ...
  downloadPdf() {
    let name = "accounts-balance-"+this.startAt.toString()+"-"+this.endAt.toString();
