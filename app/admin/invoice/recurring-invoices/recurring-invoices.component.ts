...
import { InvoiceItemCreateComponent } from '../invoice-create/invoice-item-create/invoice-item-create.component'
...
export class RecurringInvoicesComponent implements OnInit {
  ...
  isCustom: boolean;
  public maxRepeat: number = 31;
  selectedCustomFrequency = "day";
    ...
    this.entityForm.controls['repeat'].disable();
    this.entityForm.controls['customFrequency'].disable();
	}
	...
	getInvoice(){
		...
		this.formTitle = "Update invoice";
          ...
          if (result.repeatFrequency === "custom"){
            this.entityForm.controls['repeat'].enable();
            this.entityForm.controls['customFrequency'].enable();
          }
  ...
  selectSendingFrequency(id: string) {
    if (id === "custom"){
      this.entityForm.controls['repeat'].enable();
      this.entityForm.controls['customFrequency'].enable();
    }else{
  ...
  selectFrequency(id: string) {
    id === "day" ? this.maxRepeat = 31 : this.maxRepeat = 12;
    this.selectedCustomFrequency = id;
  }

  neverEndFrequency(end){
    end === true ? this.entityForm.controls['endDate'].disable() : this.entityForm.controls['endDate'].enable();
  }
  ...
  onKey(value: number) {
    if (this.selectedCustomFrequency ===  "month" ){
      if (value >= 31){
        value = 31;
        this.entityForm.get('repeat').setValue(value)
      }
    }else{
      if (value >= 12){
        value = 12;
