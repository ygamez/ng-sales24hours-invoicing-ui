...
import { LayoutService } from 'src/app/utils';
import { OrdersChart } from '../charts-panel/model/orders-chart';
...
export class LineEchartComponent implements OnInit {
  ...
  ngOnChanges(): void {
    if (this.option) {
      this.updateOrdersChartOptions(this.salesChartData);
    }
  }
  ...
  ngOnInit(): void {
    let sales: SalesChart = {
      chartLabel: ["All Sales","Paid sales", "Send sale"],
      linesData : [[100,220,256],[258,245,256],[500,220,256]]
    }
    this.salesChartData = sales;
  }
  ...
  updateOrdersChartOptions(salesChartData: OrdersChart) {
    const options = this.option;
    const series = this.getNewSeries(options.series, salesChartData.linesData);
    const xAxis = this.getNewXAxis(options.xAxis, salesChartData.chartLabel);
