...
import { LineChartReport } from 'src/app/models/dashboard-report';
...
export class OrdersChartComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  @Input()
  transactionChartData: LineChartReport;
  private alive = true;
  echartsIntance: any;
  option: any;
  ...
  ngOnChanges(): void {
    if (this.option) {
      this.updateLineChartReportOptions(this.transactionChartData);
    }
  }
  ...
  updateLineChartReportOptions(transactionChartData: LineChartReport) {
    const options = this.option;
    const series = this.getNewSeries(options.series, transactionChartData?.linesData);
    const xAxis = this.getNewXAxis(options.xAxis, transactionChartData?.chartLabel);
